/**
 * DHE Opportunity lifecycle — Closed Lost: create `TestOpp-<timestamp>` → Reservations → add product → Closed Lost.
 * Run: npm test -- tests1/create-DHE-Opp-LifeCycle-closeLost.aspx.ts
 *
 * Required in .env: SALESFORCE_USERNAME, SALESFORCE_PASSWORD
 * Optional: SALESFORCE_TEST_LOSS_REASON (e.g. Not Interested), SALESFORCE_TEST_LOSS_REASON_JUSTIFICATION
 * Flow: Edit opportunity record → set Lost Reason → Path/Close dialog → Closed Lost
 */
...
async function switchToDetailsTab(page: Page, untilVisible: { timeout: number }) {
  if (await blockingRecordModal(page).first().isVisible().catch(() => false)) return;

  const detailsTab = opportunityRecordTablist(page).getByRole('tab', { name: 'Details' });
  if ((await detailsTab.getAttribute('aria-selected')) !== 'true') {
    await detailsTab.click({ timeout: untilVisible.timeout });
    await page.waitForLoadState('domcontentloaded');
  }
}
...
function pathArticle(page: Page) {
  return page.locator('article').filter({ has: page.getByRole('heading', { name: 'Path' }) });
}

function opportunityPathListbox(page: Page) {
  return pathArticle(page).getByRole('listbox').first();
}

function opportunityStageRow(page: Page) {
  return page
    .getByRole('listitem')
    .filter({ has: page.getByText('Stage', { exact: true }) })
    .first();
}
...
async function dismissOpenDialogSave(page: Page) {
  const dialog = page.locator('[role="dialog"]').last();
  if (!(await dialog.isVisible().catch(() => false))) return;

  for (const name of ['Save', 'Submit', 'Mark Stage as Complete', 'Mark as Complete']) {
...
function scopePage(scope: Page | Locator): Page {
  if ('goto' in scope) return scope as Page;
  return (scope as Locator).page();
}

async function selectedPathStageName(page: Page): Promise<string> {
  const pathOptions = opportunityPathListbox(page);
  return (await pathOptions.getByRole('option', { selected: true }).innerText().catch(() => '')).trim();
}

async function waitForOpportunityStageClosedLost(page: Page, untilVisible: { timeout: number }) {
  await waitForPathSaveComplete(page, untilVisible.timeout);
  await closeOpportunityDialog(page).waitFor({ state: 'hidden', timeout: untilVisible.timeout }).catch(() => {});

  const stageRow = opportunityStageRow(page);
  await stageRow.waitFor({ state: 'visible', ...untilVisible });
  await expect(stageRow.getByText('Closed Lost', { exact: true })).toBeVisible({
    timeout: untilVisible.timeout,
  });
}

async function selectFirstNativeSelectOption(select: Locator, configuredLabel: string) {
  if (configuredLabel) {
    await select.selectOption({ label: configuredLabel });
    return;
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (value && label && !/select/i.test(label)) {
...
async function selectLostReasonOption(scope: Page | Locator, configuredLabel: string) {
  const page = scopePage(scope);
  const lostReason = scope.getByRole('combobox', { name: /Lost Reason/i });
  if (!(await lostReason.isVisible().catch(() => false))) return;

  const isNativeSelect = await lostReason.evaluate((el) => el.tagName.toLowerCase() === 'select').catch(() => false);
  if (isNativeSelect) {
    await selectFirstNativeSelectOption(lostReason, configuredLabel);
    return;
  }

  await lostReason.scrollIntoViewIfNeeded();
  const openTrigger = lostReason.locator('button.slds-combobox__input, button[role="combobox"]').first();
  if (await openTrigger.isVisible().catch(() => false)) {
    await openTrigger.click({ timeout: 10_000 });
  } else {
    await lostReason.click({ timeout: 10_000 });
  }

  const lostReasonListbox = page.getByRole('listbox', { name: /Lost Reason/i });
  await lostReasonListbox.waitFor({ state: 'visible', timeout: 10_000 });

  if (configuredLabel) {
    const option = lostReasonListbox
      .getByRole('option', { name: configuredLabel, exact: true })
      .or(lostReasonListbox.locator('[role="option"]').filter({ hasText: configuredLabel }).first())
      .or(page.getByTitle(configuredLabel, { exact: true }));
    await option.first().click({ timeout: 10_000 });
    return;
  }

  const options = lostReasonListbox.getByRole('option');
  const count = await options.count();
  for (let i = 0; i < count; i++) {
    const text = (await options.nth(i).innerText()).trim();
...
async function fillLostReasonFields(scope: Page | Locator, untilVisible: { timeout: number }) {
  const configuredReason = (process.env.SALESFORCE_TEST_LOSS_REASON || '').trim();
  const justification = (process.env.SALESFORCE_TEST_LOSS_REASON_JUSTIFICATION || 'Automated test closure').trim();

  await selectLostReasonOption(scope, configuredReason);

  const lostReasonJustification = scope.getByRole('textbox', { name: /Lost Reason Justification/i });
  if (await lostReasonJustification.isVisible().catch(() => false)) {
    await lostReasonJustification.click({ timeout: untilVisible.timeout });
    await lostReasonJustification.fill(justification);
  }
}

function lostReasonListItem(page: Page) {
  return page
    .getByRole('listitem')
    .filter({ has: page.getByText('Lost Reason', { exact: true }) })
    .first();
}

/** Open record Edit, set Lost Reason + Justification, and save before closing as Closed Lost. */
async function editOpportunityAndSetLostReason(page: Page, untilVisible: { timeout: number }) {
  const configuredReason = (process.env.SALESFORCE_TEST_LOSS_REASON || '').trim();
  const justification = (process.env.SALESFORCE_TEST_LOSS_REASON_JUSTIFICATION || 'Automated test closure').trim();

  await switchToDetailsTab(page, untilVisible);

  const editBtn = page.getByRole('button', { name: 'Edit', exact: true }).first();
  await editBtn.scrollIntoViewIfNeeded();
  await editBtn.click({ timeout: untilVisible.timeout });

  const editDialog = page.getByRole('dialog').last();
  await editDialog.waitFor({ state: 'visible', ...untilVisible });

  const lostReasonField = editDialog.getByRole('combobox', { name: /Lost Reason/i });
  await lostReasonField.scrollIntoViewIfNeeded().catch(() => {});
  await fillLostReasonFields(editDialog, untilVisible);

  await editDialog.getByRole('button', { name: 'Save', exact: true }).click({ timeout: untilVisible.timeout });
  await editDialog.waitFor({ state: 'hidden', timeout: untilVisible.timeout });
  await waitForPathSaveComplete(page, untilVisible.timeout);

  if (configuredReason) {
    await expect(lostReasonListItem(page).getByText(configuredReason, { exact: true })).toBeVisible({
      timeout: untilVisible.timeout,
    });
  }

  console.log(`Edit opportunity — Lost Reason: ${configuredReason || '(first available)'} — Justification: ${justification}`);
}

async function navigateToOpportunityMainView(
  page: Page,
  opportunityName: string,
  oppUrlMatch: RegExpMatchArray | null,
  ...
  await page
    ...
    .waitFor({ state: 'visible', ...untilVisible });
}
...
async function openCloseOpportunityDialogViaPath(page: Page, untilVisible: { timeout: number }) {
  ...
  if (!(await selectClosedStageBtn.isVisible().catch(() => false))) {
    await advancePathToSelectClosedStage(page, untilVisible);
  }

  await selectClosedStageBtn.waitFor({ state: 'visible', timeout: untilVisible.timeout });
  await selectClosedStageBtn.click({ timeout: untilVisible.timeout });
  await closeOpportunityDialog(page).waitFor({ state: 'visible', ...untilVisible });
}

async function advancePathToSelectClosedStage(page: Page, untilVisible: { timeout: number }) {
  await waitForPathSaveComplete(page, untilVisible.timeout);

  const article = pathArticle(page);
  await article.scrollIntoViewIfNeeded();
  const pathOptions = opportunityPathListbox(page);
  await pathOptions.waitFor({ state: 'visible', ...untilVisible });

  const selectClosedStageBtn = page.getByRole('button', { name: /Select Closed Stage/i });
  if (await selectClosedStageBtn.isVisible().catch(() => false)) return;

  const deadline = Date.now() + untilVisible.timeout;

  while (Date.now() < deadline) {
    if (await selectClosedStageBtn.isVisible().catch(() => false)) return;

    await waitForPathSaveComplete(page, Math.min(15_000, deadline - Date.now()));
    const currentStage = await selectedPathStageName(page);

    if (currentStage.includes('Closed')) {
      if (await selectClosedStageBtn.isVisible().catch(() => false)) return;
      await pathOptions.getByRole('option', { name: 'Closed', exact: true }).click({ timeout: 10_000 });
      await waitForPathSaveComplete(page, 15_000);
      if (await selectClosedStageBtn.isVisible().catch(() => false)) return;
    }

    const markCompleteBtn = article.getByRole('button', { name: /Mark Stage as Complete/i });
    if (await markCompleteBtn.isVisible().catch(() => false)) {
      await markCompleteBtn.scrollIntoViewIfNeeded();
      await markCompleteBtn.click({ timeout: 10_000 });
      await dismissOpenDialogSave(page);
      await waitForPathSaveComplete(page, 20_000);

      const afterStage = await selectedPathStageName(page);
      if (afterStage === currentStage) {
        const nextStage = currentStage.includes('In Discussion')
          ? 'Approval'
          : currentStage.includes('Approval')
            ? 'Closed'
            : null;
        if (nextStage) {
          await pathOptions.getByRole('option', { name: nextStage, exact: true }).click({ timeout: 10_000 });
          await waitForPathSaveComplete(page, 15_000);
        }
      }
      continue;
    ...
    await expect
      .poll(async () => selectClosedStageBtn.isVisible().catch(() => false), { timeout: 2_000 })
      .toBeTruthy()
      .catch(() => {});
  }

  await selectClosedStageBtn.waitFor({ state: 'visible', timeout: 5_000 });
}

async function completeCloseOpportunityDialog(page: Page, untilVisible: { timeout: number }) {
  const closeDialog = closeOpportunityDialog(page);
  await closeDialog.waitFor({ state: 'visible', ...untilVisible });

  const stageSelect = closeDialog.getByRole('combobox', { name: /^Stage/i });
  await stageSelect.waitFor({ state: 'visible', ...untilVisible });
  await stageSelect.selectOption({ label: 'Closed Lost' });
  await page.waitForLoadState('domcontentloaded').catch(() => {});
  await fillLostReasonFields(closeDialog, untilVisible);
...
async function setStageClosedLostViaEditStage(page: Page, untilVisible: { timeout: number }) {
  await waitForPathSaveComplete(page, untilVisible.timeout);

  const stageRow = opportunityStageRow(page);
  await stageRow.scrollIntoViewIfNeeded();
  await stageRow.getByRole('button', { name: /Edit Stage/i }).click({ timeout: untilVisible.timeout });

  const editPopover = page.locator('.slds-popover, [role="dialog"]').last();
  const stageCombobox = editPopover.getByRole('combobox', { name: /^Stage/i }).first();
  await stageCombobox.waitFor({ state: 'visible', ...untilVisible });
  await stageCombobox.click();
  await page.getByRole('option', { name: 'Closed Lost', exact: true }).click({ timeout: 10_000 });
  await fillLostReasonFields(page, untilVisible);

  const saveBtn = editPopover.getByRole('button', { name: 'Save', exact: true }).first();
  await saveBtn.click({ timeout: untilVisible.timeout });
  await waitForOpportunityStageClosedLost(page, untilVisible);
}
...
test.describe('Create DHE Opportunity — Closed Lost', () => {
  test.describe.configure({ retries: 0 });

  test('create opp → event time → add product → Closed Lost', async ({ page }) => {
    ...
    await runPhase('Edit opportunity: set Lost Reason', async () => {
      await navigateToOpportunityMainView(page, opportunityName, oppUrlMatch, lightningOrigin);
      await editOpportunityAndSetLostReason(page, untilVisible);
    });

    await runPhase(
      'Main view: Closed Lost',
      async () => {
        await navigateToOpportunityMainView(page, opportunityName, oppUrlMatch, lightningOrigin);
        await pathArticle(page).scrollIntoViewIfNeeded();
