/**
 * Education opportunity (Education record type): sandbox login → Home → Opportunities → New →
 * XPath span[normalize-space()='Education'] in the record-type dialog → Next → fill → Save.
 * Run: npm test -- tests1/create-Education-Opp.spec.ts
 *
 ...
 * Opportunity name is TestEducationOpp + timestamp (e.g. TestEducationOpp-1730000000000).
 *
...
const Education_RECORD_TYPE_XPATH = ".//span[normalize-space()='Education']";

/** testRigor path anchor: Education field "*From Event Date" (replaces legacy "Date of Visit" label). */
...
test.describe('Create Event Opportunity', () => {
  test('login → new Education opportunity → save', async ({ page }) => {
    ...
    const opportunityName = `TestEducationOpp-${Date.now()}`;
    ...
    await test.step('Opportunities → New → Education record type', async () => {
    ...
    const EducationRecordType = recordTypeDialog.locator(`xpath=${Education_RECORD_TYPE_XPATH}`);
    if (await EducationRecordType.first().isVisible({ timeout: 8000 }).catch(() => false)) {
      await EducationRecordType.first().click();
  ...
  /*  const subCategory = page.getByRole('combobox', { name: 'Sub-Category' });
    ...
    const corpGov = page.getByText('Education ');
   ...
   /* const nextStep = page.getByRole('textbox', { name: 'Next Step' });
    ...
    // *Asset: Education often uses a picklist combobox; DHE uses a dual listbox + "Move to Chosen".
    ...
    // *Sub-Asset: same split as Asset — combobox on Education, dual listbox on DHE-style forms.
