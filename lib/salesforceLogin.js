 ...
 * Supports OTP/code fields and WebAuthn pages (headed manual wait + PIN typing).
...
async function completeSalesforcePasskeyIfPresent(page, opts = {}) {
  ...
  const headedWaitMs = opts.headedWaitMs ?? (Number(process.env.SALESFORCE_PASSKEY_WAIT_MS) || 90_000);
  ...
  // WebAuthn passkey page: type PIN (e.g. 130986) and wait for manual completion in headed runs.
