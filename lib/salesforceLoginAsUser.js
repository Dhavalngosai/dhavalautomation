...
const { waitForSalesforceReady, waitForStable } = require('./waitHelpers');
...
async function trySearchUserInContext(ctx, userSearchText) {
  ...
  for (const search of searchLocators) {
    ...
    await input.click();
    await input.fill('');
    await input.fill(userSearchText);
