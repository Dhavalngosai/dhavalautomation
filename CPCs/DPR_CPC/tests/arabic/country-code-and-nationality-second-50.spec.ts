/**
 * DPR CPC Arabic QA – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
...
const countryCodeRange = getFieldRange(
  'DPR_CPC_ARABIC_COUNTRY_CODE_LIMIT',
  'DPR_CPC_ARABIC_COUNTRY_CODE_OFFSET',
  'DPR_CPC_ARABIC_FIELD_LIMIT',
  'DPR_CPC_ARABIC_FIELD_OFFSET'
);
const nationalityRange = getFieldRange(
  'DPR_CPC_ARABIC_NATIONALITY_LIMIT',
  'DPR_CPC_ARABIC_NATIONALITY_OFFSET',
  'DPR_CPC_ARABIC_FIELD_LIMIT',
  'DPR_CPC_ARABIC_FIELD_OFFSET'
);
...
test.describe('DPR CPC Arabic – Country code + Nationality (paired save)', () => {
  ...
  test(`set country code + nationality together, save once per pair (${codeRangeLabel})`, async ({ page }, testInfo) => {
    test.setTimeout(0);
    test.skip(!cpcUrl, 'Set DPR_CPC_ARABIC_URL in .env to the full Arabic Cloud Page link (including qs=).');
