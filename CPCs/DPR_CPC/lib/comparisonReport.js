...
const DEFAULT_ENGLISH_RESULTS_ROOT = path.join(PROJECT_ROOT, 'DPR_CPC_English_Results');
const DEFAULT_ARABIC_RESULTS_ROOT = path.join(PROJECT_ROOT, 'DPR_CPC_Arabic_Results');
const LEGACY_RESULTS_ROOT = path.join(PROJECT_ROOT, 'DPR_CPC_Results');
...
function getPackageVersion() {
  if (process.env.DPR_CPC_VERSION?.trim()) {
    return process.env.DPR_CPC_VERSION.trim();
  }
...
function getResultsRootForLocale(locale) {
  if (locale === 'Arabic') {
    return (
      process.env.DPR_CPC_ARABIC_RESULTS_DIR?.trim() ||
      process.env.DPR_CPC_RESULTS_DIR?.trim() ||
      DEFAULT_ARABIC_RESULTS_ROOT
    );
  }

  return (
    process.env.DPR_CPC_ENGLISH_RESULTS_DIR?.trim() ||
    process.env.DPR_CPC_RESULTS_DIR?.trim() ||
...
function getRunTimestampForLocale(locale) {
  if (locale === 'Arabic' && process.env.DPR_CPC_ARABIC_RUN_TIMESTAMP?.trim()) {
    return process.env.DPR_CPC_ARABIC_RUN_TIMESTAMP.trim();
  }

  if (locale === 'English' && process.env.DPR_CPC_ENGLISH_RUN_TIMESTAMP?.trim()) {
    return process.env.DPR_CPC_ENGLISH_RUN_TIMESTAMP.trim();
  }

  return process.env.DPR_CPC_RUN_TIMESTAMP?.trim() || formatRunTimestamp(new Date());
}
