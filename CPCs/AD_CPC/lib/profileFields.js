/**
 * Helpers for AD CPC My Profile fields:
 ...
 * GV (and some other CPC pages) render multiple `#profile-submit` nodes (profile + interests);
 * only the active tab's button is visible — prefer that to avoid strict-mode / "not found" failures.
 * @param {import('@playwright/test').Page} page
 */
async function clickProfileSave(page) {
  ...
  if (!visible) {
    // Ensure My Profile tab is active, then retry
