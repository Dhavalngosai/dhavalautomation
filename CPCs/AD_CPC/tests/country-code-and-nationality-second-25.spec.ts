/**
 * AD CPC – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
 * Default: second 25 pairs (items 26-50, offset 25, limit 25).
 * 
 *
 * Run:
 *   npm test -- tests/country-code-and-nationality-second-25.spec.ts
 *   run-ad-cpc-second-25.bat
 *   run-ad-cpc-second-25.bat --headed
 *
 * Env:
 *   AD_CPC_URL (optional if DEFAULT_URL still valid)
 *   AD_CPC_COUNTRY_CODE_LIMIT / AD_CPC_NATIONALITY_LIMIT / AD_CPC_FIELD_LIMIT (default 25)
 *   AD_CPC_COUNTRY_CODE_OFFSET / AD_CPC_NATIONALITY_OFFSET / AD_CPC_FIELD_OFFSET (default 25)
 */
...
const countryCodeRange = getFieldRange(
  'AD_CPC_COUNTRY_CODE_LIMIT',
  'AD_CPC_COUNTRY_CODE_OFFSET',
  'AD_CPC_FIELD_LIMIT',
  'AD_CPC_FIELD_OFFSET',
  SECOND_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'AD_CPC_NATIONALITY_LIMIT',
  'AD_CPC_NATIONALITY_OFFSET',
  'AD_CPC_FIELD_LIMIT',
  'AD_CPC_FIELD_OFFSET',
  SECOND_TWENTY_FIVE_RANGE
);
...
test.describe('AD CPC – Country code + Nationality (paired save)', () => {
