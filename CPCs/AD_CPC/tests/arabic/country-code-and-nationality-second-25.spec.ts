/**
 * AD CPC Arabic QA – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
...
const countryCodeRange = getFieldRange(
  'AD_CPC_ARABIC_COUNTRY_CODE_LIMIT',
  'AD_CPC_ARABIC_COUNTRY_CODE_OFFSET',
  'AD_CPC_ARABIC_FIELD_LIMIT',
  'AD_CPC_ARABIC_FIELD_OFFSET',
  SECOND_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'AD_CPC_ARABIC_NATIONALITY_LIMIT',
  'AD_CPC_ARABIC_NATIONALITY_OFFSET',
  'AD_CPC_ARABIC_FIELD_LIMIT',
  'AD_CPC_ARABIC_FIELD_OFFSET',
  SECOND_TWENTY_FIVE_RANGE
);
...
test.describe('AD CPC Arabic – Country code + Nationality (paired save)', () => {
  ...
  test(`set country code + nationality together, save once per pair (${codeRangeLabel})`, async ({ page }, testInfo) => {
    test.setTimeout(0);
    test.skip(!cpcUrl, 'Set AD_CPC_ARABIC_URL in .env to the full Arabic Cloud Page link (including qs=).');
