/**
 * RM CPC (Customer Preference Center) QA Cloud Page:
 * open preference URL → change Country code + Nationality → Save (My Profile).
 *
 * Run from this folder (RM_CPC):
 *   npm test
 *   npm run test:headed
 *   run-rm-cpc-update-profile.bat
 *   run-rm-cpc-update-profile.bat --headed
 *
 * Required in .env (optional if DEFAULT_URL below is still valid):
 *   RM_CPC_URL – full Cloud Page URL including qs=… (from the SFMC email link)
 *
 * Optional:
 *   RM_CPC_COUNTRY_CODE – phone dial code, e.g. +971 (default +971)
 *   RM_CPC_NATIONALITY – nationality option label, e.g. India (default India)
 *   RM_CPC_LOCATOR_TIMEOUT_MS – default 30000
 *
 * Notes:
...
const countryCode = (process.env.RM_CPC_COUNTRY_CODE || '+971').trim();
const nationality = (process.env.RM_CPC_NATIONALITY || 'India').trim();

const rawLocatorMs = Number(process.env.RM_CPC_LOCATOR_TIMEOUT_MS);
...
async function selectNationality(page: import('@playwright/test').Page, label: string): Promise<void> {
  ...
  if (count === 0) {
    throw new Error(
      `Nationality option "${label}" not found in #nationality. Set RM_CPC_NATIONALITY to an exact option label.`
    );
  }
...
test.describe('RM CPC – update Country code & Nationality', () => {
  test('change Country code and Nationality, then Save', async ({ page }) => {
    ...
    await test.step('Open RM CPC Cloud Page', async () => {
