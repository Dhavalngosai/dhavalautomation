/**
 * RM CPC – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
 * Default: ninth 25 pairs (items 201-225, offset 200, limit 25).
 * 
 *
 * Run:
 *   npm test -- tests/country-code-and-nationality-ninth-25.spec.ts
 *   run-rm-cpc-ninth-25.bat
 *   run-rm-cpc-ninth-25.bat --headed
 *
 * Env:
 *   RM_CPC_URL (optional if DEFAULT_URL still valid)
 *   RM_CPC_COUNTRY_CODE_LIMIT / RM_CPC_NATIONALITY_LIMIT / RM_CPC_FIELD_LIMIT (default 25)
 *   RM_CPC_COUNTRY_CODE_OFFSET / RM_CPC_NATIONALITY_OFFSET / RM_CPC_FIELD_OFFSET (default 75)
 */
...
const countryCodeRange = getFieldRange(
  'RM_CPC_COUNTRY_CODE_LIMIT',
  'RM_CPC_COUNTRY_CODE_OFFSET',
  'RM_CPC_FIELD_LIMIT',
  'RM_CPC_FIELD_OFFSET',
  NINTH_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'RM_CPC_NATIONALITY_LIMIT',
  'RM_CPC_NATIONALITY_OFFSET',
  'RM_CPC_FIELD_LIMIT',
  'RM_CPC_FIELD_OFFSET',
  NINTH_TWENTY_FIVE_RANGE
);
...
test.describe('RM CPC – Country code + Nationality (paired save)', () => {
