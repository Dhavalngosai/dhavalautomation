/**
 * Trex CPC – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
 * Default: eighth 25 pairs (items 176-200, offset 175, limit 25).
 * 
 *
 * Run:
 *   npm test -- tests/country-code-and-nationality-eighth-25.spec.ts
 *   run-trex-cpc-eighth-25.bat
 *   run-trex-cpc-eighth-25.bat --headed
 *
 * Env:
 *   TREX_CPC_URL (optional if DEFAULT_URL still valid)
 *   TREX_CPC_COUNTRY_CODE_LIMIT / TREX_CPC_NATIONALITY_LIMIT / TREX_CPC_FIELD_LIMIT (default 25)
 *   TREX_CPC_COUNTRY_CODE_OFFSET / TREX_CPC_NATIONALITY_OFFSET / TREX_CPC_FIELD_OFFSET (default 75)
 */
import { expect, test } from '@playwright/test';
import { attachComparisonReport } from '../lib/comparisonReport.js';
import { formatRangeLabel, getFieldRange, sliceFieldItems, EIGHTH_TWENTY_FIVE_RANGE } from '../lib/fieldRange.js';
import {
  clickProfileSave,
...
const DEFAULT_URL =
  'https://cloud.explore.trexglamping.com/TREX_CPC?qs=ABB7InYiOjEsImQiOjQ5NTF9ADMAAAAAAJlocIq4v1J9nH8SEWgAqx4llHDSyMcB_9rdnqGNdaTCKFwmKWi37KiItsQp2cktUlRa0yRXOF7HH-Rf--W6QKrZbO3gwX9CVPRZq8KcGhUfx7LQdiakgZCQ1tQ0W_vmfmcSMHWDABeiqnTL4LY';

const cpcUrl = (process.env.TREX_CPC_URL || DEFAULT_URL).trim();

const countryCodeRange = getFieldRange(
  'TREX_CPC_COUNTRY_CODE_LIMIT',
  'TREX_CPC_COUNTRY_CODE_OFFSET',
  'TREX_CPC_FIELD_LIMIT',
  'TREX_CPC_FIELD_OFFSET',
  EIGHTH_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'TREX_CPC_NATIONALITY_LIMIT',
  'TREX_CPC_NATIONALITY_OFFSET',
  'TREX_CPC_FIELD_LIMIT',
  'TREX_CPC_FIELD_OFFSET',
  EIGHTH_TWENTY_FIVE_RANGE
);
...
test.describe('Trex CPC – Country code + Nationality (paired save)', () => {
  ...
  test(`set country code + nationality together, save once per pair (${codeRangeLabel})`, async ({ page }, testInfo) => {
    test.setTimeout(0);
    test.skip(!cpcUrl, 'Set TREX_CPC_URL in .env to the full Cloud Page link (including qs=).');
