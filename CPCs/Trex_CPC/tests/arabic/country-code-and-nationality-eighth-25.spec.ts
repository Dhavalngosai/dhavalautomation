/**
 * Trex CPC Arabic QA – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
...
const countryCodeRange = getFieldRange(
  ...
  'TREX_CPC_ARABIC_COUNTRY_CODE_OFFSET',
  'TREX_CPC_ARABIC_FIELD_LIMIT',
  'TREX_CPC_ARABIC_FIELD_OFFSET',
  EIGHTH_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'TREX_CPC_ARABIC_NATIONALITY_LIMIT',
  'TREX_CPC_ARABIC_NATIONALITY_OFFSET',
  'TREX_CPC_ARABIC_FIELD_LIMIT',
  'TREX_CPC_ARABIC_FIELD_OFFSET',
  EIGHTH_TWENTY_FIVE_RANGE
);
...
test.describe('Trex CPC Arabic – Country code + Nationality (paired save)', () => {
  ...
  test(`set country code + nationality together, save once per pair (${codeRangeLabel})`, async ({ page }, testInfo) => {
    test.setTimeout(0);
    test.skip(!cpcUrl, 'Set TREX_CPC_ARABIC_URL in .env to the full Arabic Cloud Page link (including qs=).');
