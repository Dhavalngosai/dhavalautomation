/**
 * TV CPC Arabic QA – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
...
const countryCodeRange = getFieldRange(
  'TV_CPC_ARABIC_COUNTRY_CODE_LIMIT',
  'TV_CPC_ARABIC_COUNTRY_CODE_OFFSET',
  'TV_CPC_ARABIC_FIELD_LIMIT',
  'TV_CPC_ARABIC_FIELD_OFFSET',
  FOURTH_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'TV_CPC_ARABIC_NATIONALITY_LIMIT',
  'TV_CPC_ARABIC_NATIONALITY_OFFSET',
  'TV_CPC_ARABIC_FIELD_LIMIT',
  'TV_CPC_ARABIC_FIELD_OFFSET',
  FOURTH_TWENTY_FIVE_RANGE
);
...
test.describe('TV CPC Arabic – Country code + Nationality (paired save)', () => {
  ...
  test(`set country code + nationality together, save once per pair (${codeRangeLabel})`, async ({ page }, testInfo) => {
    test.setTimeout(0);
    test.skip(!cpcUrl, 'Set TV_CPC_ARABIC_URL in .env to the full Arabic Cloud Page link (including qs=).');
