...
const DEFAULT_ENGLISH_RESULTS_ROOT = path.join(PROJECT_ROOT, 'TGP_CPC_English_Results');
const DEFAULT_ARABIC_RESULTS_ROOT = path.join(PROJECT_ROOT, 'TGP_CPC_Arabic_Results');
const LEGACY_RESULTS_ROOT = path.join(PROJECT_ROOT, 'TGP_CPC_Results');
...
function getPackageVersion() {
  if (process.env.TGP_CPC_VERSION?.trim()) {
    return process.env.TGP_CPC_VERSION.trim();
  }
...
function getLocaleFromTestFile(testFile) {
  ...
  return /(^|\/)arabic\//.test(normalized) || normalized.includes('tests/arabic/')
    ? 'Arabic'
    : 'English';
}
...
function getResultsRootForLocale(locale) {
  if (locale === 'Arabic') {
    return (
      process.env.TGP_CPC_ARABIC_RESULTS_DIR?.trim() ||
      process.env.TGP_CPC_RESULTS_DIR?.trim() ||
      DEFAULT_ARABIC_RESULTS_ROOT
    );
  }

  return (
    process.env.TGP_CPC_ENGLISH_RESULTS_DIR?.trim() ||
    process.env.TGP_CPC_RESULTS_DIR?.trim() ||
...
function getRunTimestampForLocale(locale) {
  if (locale === 'Arabic' && process.env.TGP_CPC_ARABIC_RUN_TIMESTAMP?.trim()) {
    return process.env.TGP_CPC_ARABIC_RUN_TIMESTAMP.trim();
  }

  if (locale === 'English' && process.env.TGP_CPC_ENGLISH_RUN_TIMESTAMP?.trim()) {
    return process.env.TGP_CPC_ENGLISH_RUN_TIMESTAMP.trim();
  }

  return process.env.TGP_CPC_RUN_TIMESTAMP?.trim() || formatRunTimestamp(new Date());
}
...
/**
 * Pin a shared run timestamp so comparison reports and full-run archives land in one folder.
 * @param {string} [timestamp]
 * @returns {string}
 */
function ensureSharedRunTimestamp(timestamp) {
  if (!process.env.TGP_CPC_RUN_TIMESTAMP?.trim()) {
    process.env.TGP_CPC_RUN_TIMESTAMP = timestamp || formatRunTimestamp(new Date());
  }
  return process.env.TGP_CPC_RUN_TIMESTAMP.trim();
}
