/**
 * LL CPC Arabic QA – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
...
const countryCodeRange = getFieldRange(
  'LL_CPC_ARABIC_COUNTRY_CODE_LIMIT',
  'LL_CPC_ARABIC_COUNTRY_CODE_OFFSET',
  'LL_CPC_ARABIC_FIELD_LIMIT',
  'LL_CPC_ARABIC_FIELD_OFFSET',
  FIRST_FIFTY_RANGE
);
const nationalityRange = getFieldRange(
  'LL_CPC_ARABIC_NATIONALITY_LIMIT',
  'LL_CPC_ARABIC_NATIONALITY_OFFSET',
  'LL_CPC_ARABIC_FIELD_LIMIT',
  'LL_CPC_ARABIC_FIELD_OFFSET',
  FIRST_FIFTY_RANGE
);
...
test.describe('LL CPC Arabic – Country code + Nationality (paired save)', () => {
  ...
  test(`set country code + nationality together, save once per pair (${codeRangeLabel})`, async ({ page }, testInfo) => {
    test.setTimeout(0);
    test.skip(!cpcUrl, 'Set LL_CPC_ARABIC_URL in .env to the full Arabic Cloud Page link (including qs=).');
