/**
 * LL CPC – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
 * Default: first 50 pairs (items 1–50, offset 0, limit 50).
 *
 * Run:
 *   npm test -- tests/country-code-and-nationality-first-50.spec.ts
 *   run-ll-cpc-first-50.bat
 *   run-ll-cpc-first-50.bat --headed
 *
 * Env:
 *   LL_CPC_URL (optional if DEFAULT_URL still valid)
 *   LL_CPC_COUNTRY_CODE_LIMIT / LL_CPC_NATIONALITY_LIMIT / LL_CPC_FIELD_LIMIT (default 50)
 *   LL_CPC_COUNTRY_CODE_OFFSET / LL_CPC_NATIONALITY_OFFSET / LL_CPC_FIELD_OFFSET (default 0)
 */
...
const countryCodeRange = getFieldRange(
  'LL_CPC_COUNTRY_CODE_LIMIT',
  'LL_CPC_COUNTRY_CODE_OFFSET',
  'LL_CPC_FIELD_LIMIT',
  'LL_CPC_FIELD_OFFSET',
  FIRST_FIFTY_RANGE
);
const nationalityRange = getFieldRange(
  'LL_CPC_NATIONALITY_LIMIT',
  'LL_CPC_NATIONALITY_OFFSET',
  'LL_CPC_FIELD_LIMIT',
  'LL_CPC_FIELD_OFFSET',
  FIRST_FIFTY_RANGE
);
...
test.describe('LL CPC – Country code + Nationality (paired save)', () => {
