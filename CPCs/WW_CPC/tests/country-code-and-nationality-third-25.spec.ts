/**
 * WW CPC – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
 * Default: third 25 pairs (items 51-75, offset 50, limit 25).
 * 
 *
 * Run:
 *   npm test -- tests/country-code-and-nationality-third-25.spec.ts
 *   run-ww-cpc-third-25.bat
 *   run-ww-cpc-third-25.bat --headed
 *
 * Env:
 *   WW_CPC_URL (optional if DEFAULT_URL still valid)
 *   WW_CPC_COUNTRY_CODE_LIMIT / WW_CPC_NATIONALITY_LIMIT / WW_CPC_FIELD_LIMIT (default 25)
 *   WW_CPC_COUNTRY_CODE_OFFSET / WW_CPC_NATIONALITY_OFFSET / WW_CPC_FIELD_OFFSET (default 50)
 */
...
const countryCodeRange = getFieldRange(
  'WW_CPC_COUNTRY_CODE_LIMIT',
  'WW_CPC_COUNTRY_CODE_OFFSET',
  'WW_CPC_FIELD_LIMIT',
  'WW_CPC_FIELD_OFFSET',
  THIRD_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'WW_CPC_NATIONALITY_LIMIT',
  'WW_CPC_NATIONALITY_OFFSET',
  'WW_CPC_FIELD_LIMIT',
  'WW_CPC_FIELD_OFFSET',
  THIRD_TWENTY_FIVE_RANGE
);
...
test.describe('WW CPC – Country code + Nationality (paired save)', () => {
