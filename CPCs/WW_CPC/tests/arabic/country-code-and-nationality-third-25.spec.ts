/**
 * WW CPC Arabic QA – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
...
const countryCodeRange = getFieldRange(
  'WW_CPC_ARABIC_COUNTRY_CODE_LIMIT',
  'WW_CPC_ARABIC_COUNTRY_CODE_OFFSET',
  'WW_CPC_ARABIC_FIELD_LIMIT',
  'WW_CPC_ARABIC_FIELD_OFFSET',
  THIRD_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'WW_CPC_ARABIC_NATIONALITY_LIMIT',
  'WW_CPC_ARABIC_NATIONALITY_OFFSET',
  'WW_CPC_ARABIC_FIELD_LIMIT',
  'WW_CPC_ARABIC_FIELD_OFFSET',
  THIRD_TWENTY_FIVE_RANGE
);
...
test.describe('WW CPC Arabic – Country code + Nationality (paired save)', () => {
  ...
  test(`set country code + nationality together, save once per pair (${codeRangeLabel})`, async ({ page }, testInfo) => {
    test.setTimeout(0);
    test.skip(!cpcUrl, 'Set WW_CPC_ARABIC_URL in .env to the full Arabic Cloud Page link (including qs=).');
