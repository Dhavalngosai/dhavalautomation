/**
 * GV CPC Arabic QA – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
...
const countryCodeRange = getFieldRange(
  'GV_CPC_ARABIC_COUNTRY_CODE_LIMIT',
  'GV_CPC_ARABIC_COUNTRY_CODE_OFFSET',
  'GV_CPC_ARABIC_FIELD_LIMIT',
  'GV_CPC_ARABIC_FIELD_OFFSET',
  FIRST_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'GV_CPC_ARABIC_NATIONALITY_LIMIT',
  'GV_CPC_ARABIC_NATIONALITY_OFFSET',
  'GV_CPC_ARABIC_FIELD_LIMIT',
  'GV_CPC_ARABIC_FIELD_OFFSET',
  FIRST_TWENTY_FIVE_RANGE
);
...
test.describe('GV CPC Arabic – Country code + Nationality (paired save)', () => {
  ...
  test(`set country code + nationality together, save once per pair (${codeRangeLabel})`, async ({ page }, testInfo) => {
    test.setTimeout(0);
    test.skip(!cpcUrl, 'Set GV_CPC_ARABIC_URL in .env to the full Arabic Cloud Page link (including qs=).');
