/**
 * GV CPC – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
 * Default: first 25 pairs (items 1-25, offset 0, limit 25).
 * 
 *
 * Run:
 *   npm test -- tests/country-code-and-nationality-first-25.spec.ts
 *   run-gv-cpc-first-25.bat
 *   run-gv-cpc-first-25.bat --headed
 *
 * Env:
 *   GV_CPC_URL (optional if DEFAULT_URL still valid)
 *   GV_CPC_COUNTRY_CODE_LIMIT / GV_CPC_NATIONALITY_LIMIT / GV_CPC_FIELD_LIMIT (default 25)
 *   GV_CPC_COUNTRY_CODE_OFFSET / GV_CPC_NATIONALITY_OFFSET / GV_CPC_FIELD_OFFSET (default 0)
 */
...
const countryCodeRange = getFieldRange(
  'GV_CPC_COUNTRY_CODE_LIMIT',
  'GV_CPC_COUNTRY_CODE_OFFSET',
  'GV_CPC_FIELD_LIMIT',
  'GV_CPC_FIELD_OFFSET',
  FIRST_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'GV_CPC_NATIONALITY_LIMIT',
  'GV_CPC_NATIONALITY_OFFSET',
  'GV_CPC_FIELD_LIMIT',
  'GV_CPC_FIELD_OFFSET',
  FIRST_TWENTY_FIVE_RANGE
);
...
test.describe('GV CPC – Country code + Nationality (paired save)', () => {
