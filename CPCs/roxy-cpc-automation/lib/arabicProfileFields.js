/**
 * Roxy Live CPC Arabic – My Profile field helpers (Arabic labels; Select2-backed Country of Residence).
 */
const {
  getCountryCodes,
  setCountryCode,
...
/**
 * Select Country of Residence via Select2 UI (preferred on Arabic Roxy CPC).
 * @param {import('@playwright/test').Page} page
 * @param {string} label
 */
...
async function clickProfileSave(page) {
  const byId = page.locator('#profile-submit');
  if (await byId.isVisible().catch(() => false)) {
    await byId.click({ noWaitAfter: true });
    await page.waitForLoadState('networkidle', { timeout: 20_000 }).catch(() => undefined);
    return 'Saved Successfully';
  }

  const byLabel = page.getByRole('button', { name: 'حفظ' });
  if (await byLabel.count()) {
    await byLabel.first().click({ noWaitAfter: true });
    await page.waitForLoadState('networkidle', { timeout: 20_000 }).catch(() => undefined);
    return 'Saved Successfully';
  }
