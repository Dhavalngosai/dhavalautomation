/**
 * OB CPC Arabic QA – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
...
const countryCodeRange = getFieldRange(
  'OB_CPC_ARABIC_COUNTRY_CODE_LIMIT',
  'OB_CPC_ARABIC_COUNTRY_CODE_OFFSET',
  'OB_CPC_ARABIC_FIELD_LIMIT',
  'OB_CPC_ARABIC_FIELD_OFFSET',
  SIXTH_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'OB_CPC_ARABIC_NATIONALITY_LIMIT',
  'OB_CPC_ARABIC_NATIONALITY_OFFSET',
  'OB_CPC_ARABIC_FIELD_LIMIT',
  'OB_CPC_ARABIC_FIELD_OFFSET',
  SIXTH_TWENTY_FIVE_RANGE
);
...
test.describe('OB CPC Arabic – Country code + Nationality (paired save)', () => {
  ...
  test(`set country code + nationality together, save once per pair (${codeRangeLabel})`, async ({ page }, testInfo) => {
    test.setTimeout(0);
    test.skip(!cpcUrl, 'Set OB_CPC_ARABIC_URL in .env to the full Arabic Cloud Page link (including qs=).');
