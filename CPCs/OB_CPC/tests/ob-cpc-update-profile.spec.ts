/**
 * OB CPC (Customer Preference Center) QA Cloud Page:
 * open preference URL → change Country code + Nationality → Save (My Profile).
 *
 * Run from this folder (OB_CPC):
 *   npm test
 *   npm run test:headed
 *   run-ob-cpc-update-profile.bat
 *   run-ob-cpc-update-profile.bat --headed
 *
 * Required in .env (optional if DEFAULT_URL below is still valid):
 *   OB_CPC_URL – full Cloud Page URL including qs=… (from the SFMC email link)
 *
 * Optional:
 *   OB_CPC_COUNTRY_CODE – phone dial code, e.g. +971 (default +971)
 *   OB_CPC_NATIONALITY – nationality option label, e.g. India (default India)
 *   OB_CPC_LOCATOR_TIMEOUT_MS – default 30000
 *
 * Notes:
...
const countryCode = (process.env.OB_CPC_COUNTRY_CODE || '+971').trim();
const nationality = (process.env.OB_CPC_NATIONALITY || 'India').trim();

const rawLocatorMs = Number(process.env.OB_CPC_LOCATOR_TIMEOUT_MS);
...
async function selectNationality(page: import('@playwright/test').Page, label: string): Promise<void> {
  ...
  if (count === 0) {
    throw new Error(
      `Nationality option "${label}" not found in #nationality. Set OB_CPC_NATIONALITY to an exact option label.`
    );
  }
...
test.describe('OB CPC – update Country code & Nationality', () => {
  test('change Country code and Nationality, then Save', async ({ page }) => {
    ...
    await test.step('Open OB CPC Cloud Page', async () => {
