/**
 * RL CPC Arabic QA – Country code + Nationality (offset + limit): set both fields → Save once → comparison row.
...
const countryCodeRange = getFieldRange(
  'RL_CPC_ARABIC_COUNTRY_CODE_LIMIT',
  'RL_CPC_ARABIC_COUNTRY_CODE_OFFSET',
  'RL_CPC_ARABIC_FIELD_LIMIT',
  'RL_CPC_ARABIC_FIELD_OFFSET',
  SEVENTH_TWENTY_FIVE_RANGE
);
const nationalityRange = getFieldRange(
  'RL_CPC_ARABIC_NATIONALITY_LIMIT',
  'RL_CPC_ARABIC_NATIONALITY_OFFSET',
  'RL_CPC_ARABIC_FIELD_LIMIT',
  'RL_CPC_ARABIC_FIELD_OFFSET',
  SEVENTH_TWENTY_FIVE_RANGE
);
...
test.describe('RL CPC Arabic – Country code + Nationality (paired save)', () => {
  ...
  test(`set country code + nationality together, save once per pair (${codeRangeLabel})`, async ({ page }, testInfo) => {
    test.setTimeout(0);
    test.skip(!cpcUrl, 'Set RL_CPC_ARABIC_URL in .env to the full Arabic Cloud Page link (including qs=).');
